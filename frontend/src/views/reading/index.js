...
function getReadingExercises(examData) {
  const schema = examData.schema?.find(s => s.category === 'Reading')
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <p style={{ fontSize: 'var(--text-base)', color: 'var(--color-text-secondary)', margin: 0 }}>
          You answered all {total} reading questions. Great work!
        </p>
      </div>
...
export default function ReadingView() {
  ...
  const exercises = getReadingExercises(examData)
  ...
  return (
    <ReadingExerciseView key={key} exercises={exercises} xpReward={XP_REWARD}
