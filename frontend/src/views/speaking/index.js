...
function getSpeakingExercises(examData) {
  const schema = examData.schema?.find(s => s.category === 'Speaking')
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <p style={{ fontSize: 'var(--text-base)', color: 'var(--color-text-secondary)', margin: 0 }}>
          You completed all {total} speaking exercises. Great work!
        </p>
      </div>
...
export default function SpeakingView() {
  ...
  const exercises = getSpeakingExercises(examData)
  ...
  return (
    <SpeakingExerciseView key={key} exercises={exercises} xpReward={XP_REWARD}
